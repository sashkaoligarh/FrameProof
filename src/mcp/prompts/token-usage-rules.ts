...
ALWAYS:
- Import .figma/design-system.css in your project (generated by get_css_variables)
- Use CSS custom properties from the design system
- Match Figma values exactly (no approximation)
- If a value has no matching token, check token_hints for the nearest match
- If token_hints shows delta <= 2, use the nearest token (likely designer rounding error)
- If delta > 2, report it to the developer — it may be intentional
